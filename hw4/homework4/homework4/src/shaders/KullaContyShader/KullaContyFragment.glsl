...
vec3 fresnelSchlick(vec3 F0, vec3 V, vec3 H)
{
    ...
    vec3 ret = F0 + (1.0 - F0) * pow(1.0 - max(dot(H, V), 0.0), 5.0);
    return ret;
}
...
vec3 MultiScatterBRDF(float NdotL, float NdotV)
{
  ...
  vec3 f_add_nom = F_avg * E_avg;
  vec3 f_add_denom = 1.0 - F_avg * (1.0 - E_avg);
  vec3 f_ms_nom = (1.0 - E_o) * (1.0 - E_i);
  vec3 f_ms_denom = PI * (1.0 - E_avg);

  return f_add_nom * f_ms_nom / f_add_denom / f_ms_denom;
